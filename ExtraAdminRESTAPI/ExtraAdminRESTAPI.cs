...
namespace extraAdminREST
{
    ...
    public class ExtraAdminREST : TerrariaPlugin
    {
        ...
        {
            Commands.ChatCommands.Add(new Command(Permissions.canchat, ShowColor, "color") { HelpText = "show color of player" });
  

            TShock.RestApi.Register(new RestCommand("/AdminREST/version", AdminRest));
            TShock.RestApi.Register(new SecureRestCommand("/AdminREST/getPlayerData", getPlayerData));
            ...
            TShock.RestApi.Register(new RestCommand("/AdminREST/updateInventory", updateInventory));
             TShock.RestApi.Register(new RestCommand("/userlist", UserList));
  //          TShock.RestApi.Register(new SecureRestCommand("/SSC/updateSSC", updateSSC, "ssc.get.inventory"));
            //           TShock.RestApi.Register(new SecureRestCommand("/SSC/getSSCTransferLog", GetInventoryLog, "ssc.get.inventory"));
            TShock.RestApi.Register(new SecureRestCommand("/SSC/getInventory", getInventory, "ssc.get.inventory"));
	...
	//        public static Object getLog(RestVerbs verbs, IParameterCollection parameters, SecureRest.TokenData tokenData)
        {
...
//            Console.WriteLine(TShock.Config.LogPath + "/" + myFile);
            FileStream logFileStream = new FileStream(TShock.Config.LogPath + "/" + myFile, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
            ...
            return new RestObject() { { "file", myFile }, { "log", LogLinesEnd } };
 ...
 //          private object getPlayerData(RestVerbs verbs, IParameterCollection parameters, SecureRest.TokenData tokenData)
        {
 ...
 /*      private object updateSSC(RestRequestArgs args)
        {
    
            string inventoryString = parameters["SSC"];
            if (inventoryString == null)
                return RestError("Missing or empty inventory string - /updateSSC Account=<account id> Me=<change user> User=<SSC user> SSC=<inventory string> - item, count, prefix~");

            string username = parameters["user"];
            if (username == null)
                return RestError("Missing or empty username - /updateSSC Account=<account id> Me=<change user> user=<SSC user> SSC=<inventory string> - item, count, prefix~");

            string me = parameters["me"];
            if (me == null)
                return RestError("Missing or empty username - /updateSSC Account=<account id> me=<change user> User=<SSC user> SSC=<inventory string> - item, count, prefix~");
            ...
            try
            {
                account = Convert.ToInt32(parameters["Account"]);
            }
            catch (OverflowException)
            {
                Console.WriteLine("{0} is outside the range of the Int32 type.", parameters["Account"]);
                return RestError("Missing or empty username - /updateSSC Account=<account id> Me=<change user> User=<SSC user> SSC=<inventory string> - item, count, prefix~");
            }
            catch (FormatException)
            {
                Console.WriteLine("The value '{1}' is not in a recognizable format.", parameters["Account"]);
                return RestError("Missing or empty username - /updateSSC Account=<account id> Me=<change user> User=<SSC user> SSC=<inventory string> - item, count, prefix~");
            }
            ...
            SSC = new SSCTransfer(DB);
            //      public bool AddSSCInventory(string account, string user = "", string OldInventory = "", string NewInventory = "", string changedBy = "", bool exceptions = false)

            SSC.ModifySSCInventory(account, username, inventoryString, me, false);

            return new RestObject { { "update", inventoryString } };
           
        }
  */          
        /*
                private object GetInventoryLog(RestVerbs verbs, IParameterCollection parameters, SecureRest.TokenData tokenData)
                {

                    string sql = Path.Combine(SavePath, "tshock.sqlite");
                    DB = new SqliteConnection(string.Format("uri=file://{0},Version=3", sql));
                    SSC = new SSCTransfer(DB);
                    List<SSCInventory> si = SSC.GetSSCInventoryLog();
                    return new RestObject() { {"SSCInventoryLog", si}
                    };
       ...
       protected override void Dispose(bool disposing)
        ...
        private static List<UserList> FindUsers(string search, bool casesensitive = true)
        {
            ...
            try
            {
                sql = "SELECT * FROM Users " + search + " order by UserName Limit 300";
                ...
                using (var reader = TShock.DB.QueryReader("SELECT * FROM tsCharacter where account =@0", account))
                {
                    if (reader.Read())
                    {
                        rec = new SSCInventory(reader.Get<Int32>("Account"), reader.Get<string>("Inventory"),
                           reader.Get<Int32>("hair"), reader.Get<Int32>("hairdye"), reader.Get<Int32>("haircolor"),
                           reader.Get<Int32>("pantscolor"), reader.Get<Int32>("shirtcolor"), reader.Get<Int32>("undershirtcolor"), reader.Get<Int32>("shoecolor"),
                           reader.Get<Int32>("skincolor"), reader.Get<Int32>("eyecolor")
                           );
    ...
    public class SSCInventory
    {
        ...
        public Int32 PantsColor { get; set; }
        public Int32 ShirtColor { get; set; }
        public Int32 UnderShirtColor { get; set; }
        public Int32 ShoeColor { get; set; }
        public Int32 SkinColor { get; set; }
        public Int32 EyeColor { get; set; }

        public SSCInventory(Int32 id, string inventory, Int32 hair, Int32 hairdye, Int32 haircolor,
            Int32 pantscolor, Int32 shirtcolor, Int32 undershirtcolor, Int32 shoecolor, Int32 skincolor, Int32 eyecolor)
        {
            Id = id;
            Inventory = inventory;
            Hair = hair;
